...
drain3 is pure Python and imposes no interpreter floor, but it does constrain the environment it is installed into. Version 0.9.11 is the newest release, it ships as a source distribution only, and it pins `jsonpickle==1.5.1` and `cachetools==4.2.1` exactly. Any project that needs a different version of either library cannot install wazuhcoverage alongside it, which is the main reason to install the CLI with `pipx` rather than into a shared environment. `tools/check_dependency_pins.py` reports these pins and fails when the set changes, and CI runs it on every supported interpreter together with `pip check` and a from-scratch resolution.
